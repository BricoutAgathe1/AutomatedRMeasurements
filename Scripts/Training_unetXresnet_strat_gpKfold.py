...
class UltrasoundDataset(Dataset):
    def __init__(self, image_dirs, mask_dirs, image_transform=None, mask_transform=None):
        ...
        for image_dir in self.image_dirs:
            self.image_filenames.extend(sorted([
...
strat_labels = []
group_labels = []
for img_pth in full_dataset.image_filenames:
    img_name = os.path.basename(img_pth)
    scanner, probe = parse_filename(img_name)
    strat_labels.append(f"{scanner}_{probe}") # stratify by probe
    group_labels.append(scanner) # group by scanner
...
def save_predictions(images, masks, preds, filenames, save_dir):
    ...
    for i in range(len(images)):
        ...
        cv2.imwrite(f"{save_dir}/{name}_image.png", img * 255)
        cv2.imwrite(f"{save_dir}/{name}_gt.png", gt * 255)
        cv2.imwrite(f"{save_dir}/{name}_pred.png", pr_bin * 255)

def create_overlay(img, gt, pr):
    img_rgb = np.stack([img, img, img], axis=-1)  # Convert grayscale to RGB
    overlay = img_rgb.copy()

    overlay[gt == 1] = [0, 255, 0]  # Green for GT
    overlay[pr == 1] = [255, 0, 0]  # Red for Pred
    overlap = (gt == 1) & (pr == 1)
...
for fold, (train_idx, test_idx) in enumerate(kf.split(indices, strat_labels, group_labels)):
    print(f"\n===== FOLD {fold+1} =====")
    ...
    inner_kf = StratifiedGroupKFold(n_splits=3, shuffle=True, random_state=42+fold)
    inner_strat = [strat_labels[i] for i in train_val_idx]
    inner_groups = [group_labels[i] for i in train_val_idx]
    inner_indices = np.arange(len(train_val_idx))
    train_sub_idx, val_sub_idx = next(
        inner_kf.split(inner_indices, inner_strat, inner_groups)
    )
    ...
    print("Train scanners:", set([group_labels[i] for i in train_idx]))
    print("Test scanners:", set([group_labels[i] for i in test_idx]))
    ...
    best_epoch = -1
    best_model_path = f'../Model weights/StratifiedGroupKFold/fold_{fold}_best.pth'
    ...
    saved_figures_dirs = f'predictions_fold_{fold}'
    os.makedirs(saved_figures_dirs, exist_ok=True)
    os.makedirs(f"{saved_figures_dirs}/overlay", exist_ok=True)
    os.makedirs(f"{saved_figures_dirs}/uncertainty", exist_ok=True)
    os.makedirs(f"{saved_figures_dirs}/prob", exist_ok=True)
    ...
    with torch.no_grad():
        for images, masks, names in test_loader:
            ...
            for i in range(images.shape[0]):
                ...
                prob_map = probs_np[1]  # foreground
                ...
                # --- Save prediction ---
                cv2.imwrite(f"{saved_figures_dirs}/{name}_prediction.png", pred_bin * 255)

                # --- Save probability ---
                prob_img = np.clip(prob_map, 0, 1)
                cv2.imwrite(f"{saved_figures_dirs}/prob/{name}_prob.png", (prob_img * 255).astype(np.uint8))
                np.save(f"{saved_figures_dirs}/prob/{name}_probs_full.npy", probs_np)

                # --- Entropy uncertainty ---
                eps = 1e-8
                entropy = -np.sum(probs_np * np.log(probs_np + eps), axis=0)
                entropy_norm = (entropy - entropy.min()) / (entropy.max() - entropy.min() + eps)

                cv2.imwrite(
                    f"{saved_figures_dirs}/uncertainty/{name}_uncertainty.png",
                    (entropy_norm * 255).astype(np.uint8)
                )

                # --- Heatmap ---
                heatmap = cv2.applyColorMap((prob_img * 255).astype(np.uint8), cv2.COLORMAP_JET)
                cv2.imwrite(f"{saved_figures_dirs}/prob/{name}_heatmap.png", heatmap)

                # --- Overlay ---
                img_np = np.clip(img_np, 0, 1)
                # If img_np has shape [C, H, W], take first channel
                if img_np.ndim == 3:
                    img_grey = img_np[0]  # take channel 0
                else:
                    img_grey = img_np

                img_grey = np.clip(img_grey * 255, 0, 255).astype(np.uint8)

                # Now convert safely
                img_rgb = cv2.cvtColor(img_grey, cv2.COLOR_GRAY2RGB)

                uncertainty_coloured = cv2.applyColorMap(
                    (entropy_norm * 255).astype(np.uint8),
                    cv2.COLORMAP_JET
                )

                overlay_uncertainty = cv2.addWeighted(img_rgb, 0.5, uncertainty_coloured, 0.5, 0)
                cv2.imwrite(f"{saved_figures_dirs}/overlay/{name}_overlay_uncertainty.png", overlay_uncertainty)

                overlay = create_overlay(img_grey, mask_bin, pred_bin)
                cv2.imwrite(f"{saved_figures_dirs}/overlay/{name}_overlay.png", overlay)
