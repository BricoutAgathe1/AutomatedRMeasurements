...
for img_pth in full_dataset.image_filenames:
    ...
    probe = parse_filename(img_name)
    strat_labels.append(f"{probe}") # stratify by probe
...
for fold, (train_idx, test_idx) in enumerate(kf.split(indices, strat_labels)):
    print(f"\n===== FOLD {fold+1} =====")
    ...
    print("Train scanners:", set([strat_labels[i] for i in train_idx]))
    print("Test scanners:", set([strat_labels[i] for i in test_idx]))
    ...
    best_epoch = -1
    best_model_path = f'../Model weights/StratifiedKFold/fold_{fold}_best.pth'
