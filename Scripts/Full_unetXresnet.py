...
def segment_images_batch(input_dir, output_dir, model, device, transform, batch_size=16):
    ...
    with open('../Distances/distances_unetXresnet18.json', 'w') as f:
...
def processing_phase(data_dir, cropping_coordinates, conversion_factors, model, device, transform):
    ...
    for video_folder in tqdm(video_folders, desc="Processing video folders"):
        ...
        if video_folder in cropping_coordinates:
            ...
            with open(os.path.join(video_folder_path, 'distances_unetXresnet18.json'), 'w') as f:
            ...
            with open(os.path.join(video_folder_path, 'special_distances_unetXresnet18.json'), 'w') as f:
...
def main():
    ...
    with Pool(processes=os.cpu_count()) as pool:
        list(tqdm(pool.starmap(extract_frames, zip(video_paths, frames_dir)), desc="Extracting frames", total=len(video_files)))
...
if __name__ == "__main__":
    ...
    print("--- Testing complete: %.2f seconds ---" % (time.time() - start_time))